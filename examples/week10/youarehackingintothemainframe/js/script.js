...
function keyPressed (event) {
  ...
  var char = code[currentLine].charAt(currentChar);
...
function startDrums() {
  ...
  // Our drum loop
  var drums = new EDrums('x.xox-x.x.xoxxx-',1/16);
}
...
function createDialog () {
  ...
  // in a range.
  var x = _.random(0,$(window).width());
  var y = _.random(0,$(window).height());

  // Generate the element for the dialog
  // This HTML is just taken from jQuery UI example code
  var dialog = $('<div title="HACKING"><p><span class="ui-icon ui-icon-alert" style="float: left; margin:12px 12px 20px 0;"></span>Are you sure you want to hack this hard?<p></div>')
